	...
	return (
		...
		a.validator.addMethod(
			...
			function(a, b) {
				return (
					this.optional(b) ||
					/^([A-Z]{6}[A-Z2-9][A-NP-Z1-9])(X{3}|[A-WY-Z0-9][A-Z0-9]{2})?$/.test(a.toUpperCase())
				);
			},
								...
								: 8 & d && /^(3(0[012345]|[68]))/.test(a)
									...
									: 16 & d && /^(2(014|149))/.test(a)
										...
										: 32 & d && /^(6011)/.test(a)
											...
											: 64 & d && /^(3)/.test(a)
												? 16 === a.length
												: 64 & d && /^(2131|1800)/.test(a) ? 15 === a.length : !!(128 & d)
				);
			},
			'Please enter a valid credit card number.'
		),
		...
		a.validator.addMethod(
			...
			function(a, b) {
				return (
					this.optional(b) ||
					/^(0?[1-9]|[12]\d|3[01])[\.\/\-](0?[1-9]|1[012])[\.\/\-]([12]\d)?(\d\d)$/.test(a)
				);
			},
			a.validator.messages.date
		),
		...
		a.validator.addMethod(
			...
			function(a, b) {
				return (
					...
					this.optional(b) ||
						(a.length > 9 &&
							a.match(/^(?:(?:(?:00\s?|\+)44\s?|0)7(?:[1345789]\d{2}|624)\s?\d{3}\s?\d{3})$/))
				);
			},
			'Please specify a valid mobile number'
		),
				...
				return (
					...
					!!a.match('((^[A-Z]{1}[0-9]{7}[A-Z0-9]{1}$|^[T]{1}[A-Z0-9]{8}$)|^[0-9]{8}[A-Z]{1}$)') &&
						(/^[0-9]{8}[A-Z]{1}$/.test(a)
							...
							: !!/^[KLM]{1}/.test(a) &&
								a[8] === 'TRWAGMYFPDXBNJZSQVHLCKE'.charAt(a.substring(8, 1) % 23)))
				);
			},
		...
		a.validator.addMethod(
			...
			function(a, b) {
				return (
					this.optional(b) ||
					/^[ABCEGHJKLMNPRSTVXY]\d[ABCEGHJKLMNPRSTVWXYZ] *\d[ABCEGHJKLMNPRSTVWXYZ]\d$/i.test(a)
				);
			},
